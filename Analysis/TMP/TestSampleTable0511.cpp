...
int main()
{
  ...
  for(int i=sd_s;i<sd_e;i++) {
	...
	cnt+=1;
      ...
      stringstream ssphi;
      ostringstream ss_a;
      double phijval[5]={0,0,0,0,0},eigval[8]={0,0,0,0,0,0,0,0},codval[2]={0,0}; //dummy variable
      string str_a="";
      while(getline(ifs1,str1)) {
	...
	if(ifs2.fail()) {
	  ...
	  while(getline(ifs2,str2)) {
	    sscanf(str2.data(),"%lf %lf %lf %lf %lf %lf %lf %lf",&eigval[0],&eigval[1],&eigval[2],&eigval[3],&eigval[4],&eigval[5],&eigval[6],&eigval[7]);
	    ofs2<<setprecision(10)<<i<<' '<<phi[j]-PHIJ<<' '<<eigval[0]<<' '<<eigval[1]<<' '<<eigval[2]<<' '<<eigval[3]<<' '<<eigval[4]<<' '<<eigval[5]<<' '<<eigval[6]<<' '<<eigval[7]<<endl;
	  }
	}
	if(ifs3.fail()) {
