...
int main(int argc, char* argv[])
{
  ...
  if(ifs.fail()) {
    ...
    int C=0,cumC=0;
    double a=0.0,b=0.0,c=0.0,d=0.0,e=0.0,f=0.0,EPS=numeric_limits<double>::epsilon();
	...
	C++;
      ...
      */
      a=b=c=d=e=f=0.0;
      double cum=1.0/cumC,Range_e,Range_s,Range_m,Range_sinit,Range_sln10,Range_mln10; //Rangetmpはbinの始点,Rangeはbinの終点.
	...
	if(fabs(a)>EPS)
	  ...
	  //ゼロ固有値process
	  if(cnt==0) {
	    binstat::ModifyBI_IPRs(ipr_all,ipr_1,ipr_2,c,d,e,cnttmp);
	  }
	  else
	    binstat::innerBINprocess(ipr_all,ipr_1,ipr_2,c,d,e,Range_s,Range_e,Range_m,cnttmp,C,ofs,cnt,cum,cumC,N);
	}
	...
	if(cumC>=1) {
	  if(cumC==1) {//初めて,ゼロじゃない固有値が出てきたとき
	    ...
	    binstat::ResetBI_IPRs(ipr_all,ipr_1,ipr_2,c,d,e,cnttmp);
	  }
		...
		//bin更新処理
		binstat::updateBINprocess(ipr_all,ipr_1,ipr_2,b,c,d,e,Range_s,Range_e,Range_m,Range_sln10,Range_mln10,cnttmp,C,binrange,ofs,cnt,cum,cumC,N);
	      }
	    }
	  }
	}
  ...
  void ZeroEigenValprocess(double S,double E,double M,int zeroeig,double allC,ofstream &ofs,int N)
{
  ...
  ofs<<M<<" "<<0<<" "<<0<<" "<<0<<" "<<0<<" "<<0<<" "<<0<<" "<<0<<" "<<allC_inv*zeroeig*deltaomega_inv<<" "<<0<<" "<<zeroeig<<" "<<N<<endl;
  ...
  void innerBINprocess(iprset &ipr,iprset &ipr1,iprset &ipr2,double c,double d,double e,double S,double E,double M,int &tmpC,double allC,ofstream &ofs,int cnt,double cum,int cumcnt,int N) {
    ...
    ModifyBI_IPRs(ipr,ipr1,ipr2,c,d,e,tmpC);
    if(cnt==allC-1) {
      double T=1.0/tmpC;
      double meanc=T*ipr.sum,meanc2=ipr.sgm/tmpC;
      double meand=T*ipr1.sum,meane=T*ipr2.sum;
      double deltaomega_inv=1.0/(E-S),allC_inv=1.0/allC;
      //FILE末端ならば書き込み sqrtに意味がない.
      ofs<<M<<" "<<meanc<<" "<<ipr.min<<" "<<ipr.max<<" "<<sqrt((meanc2-meanc*meanc))<<" "<<meand<<" "<<meane<<" "<<tmpC*cum*deltaomega_inv<<" "<<allC_inv*tmpC*deltaomega_inv<<" "<<(cumcnt)*cum<<" "<<tmpC<<" "<<N<<endl;
      //bin代表点<<IPR平均値<<IPRmin値<<IPRmax値<<標準偏差<<ゼロ固有値なし分布<<ゼロ固有値あり分布<<ゼロ固有値なし累積分布<<binの中に入った固有振動数の数
    }
  }
  ...
  void updateBINprocess(iprset &ipr,iprset &ipr1,iprset &ipr2,double b,double c,double d,double e,double &S,double &E,double &M,double &Sln10,double &Mln10,int &tmpC,double allC,double range,ofstream &ofs,int cnt,double cum,int cumcnt,int N) {
    //cout<<"update"<<endl;
    double T=1.0/tmpC;
    double meanc=T*ipr.sum,meanc2=T*ipr.sgm;
    double meand=T*ipr1.sum,meane=T*ipr2.sum;
    double deltaomega_inv=1.0/(E-S),allC_inv=1.0/allC;
	...
	exit(1);
      ...
      ResetBI_IPRs(ipr,ipr1,ipr2, c,d,e,tmpC);
  ...
  void InitializeIPRparameter(double c,double &sumc,double &minc,double &maxc,double &sgmc) {
    minc=maxc=sumc=c;
    sgmc=c*c;
  }
  ...
  void ModifyIPRparameter(double c,double &sumc,double &minc,double &maxc,double &sgmc) {
    sumc+=c;
    if(minc>c)
      minc=c;
    if(maxc<c)
