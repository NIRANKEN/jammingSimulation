...
namespace JT
{
  ...
  inline void simErrWrite(stringstream &ss,ofstream &ofs,Vec_I_DP &z, int dim,int PN,DP PHI,DP rt,int Prt,int snum,int iter,DP pot,int VViter,bool eig,bool ipr,bool jacobi);
...
namespace JT
{
  ...
  inline void simErrWrite(stringstream &ss,ofstream &ofs,Vec_I_DP &z, int dim,int PN,DP PHI,DP rt,int Prt,int snum,int iter,DP pot,int VViter,bool eig,bool ipr,bool jacobi) {
    ...
    ofs2<<setprecision(10)<<dim<<' '<<PN<<' '<<PHI<<' '<<rt<<' '<<Prt<<endl;
    for(int i=0;i<PN;i++) {
