...
namespace JT
{
  ...
  void WriteEigInfo(Mat_I_DP &EigMat,Vec_I_DP &Eigval,int num,DP phi,int D,bool &IPRerror);
  void WriteEigInfo(Mat_I_DP &EigMat,Vec_I_DP &Eigval,int num,DP phi,int D,int Prt1,int Prt2,bool &IPRerror);
  void WriteEigInfo_test(Mat_I_DP &EigMat,Vec_I_DP &Eigval,int num,DP phi,int D,bool &IPRerror);
  void WriteEigInfo_test(Mat_I_DP &EigMat,Vec_I_DP &Eigval,int num,DP phi,int D,int Prt1,int Prt2,bool &IPRerror);
  void deriveEigvec(Vec_I_DP &zahyo,DP Len,DP R,DP rt,DP alp,int Prt,Vec_O_DP &out,DP &eigval,int D,int vecnum);
  void deriveEigvec(Vec_I_DP &zahyo,DP Len,DP R,DP rt,DP alp,int Prt1,int Prt2,Vec_O_DP &out,DP &eigval,int D,int vecnum);
  void outputPEL(Vec_I_DP &zahyo,DP Len,DP R,DP rt,DP alp,int Prt,int num,DP phi,Vec_I_DP &eigvec,int D,int vecnum,DP func(Vec_IO_DP,DP,DP,DP,int,int,DP));
  ...
  namespace dim2
  {
    void ModeCalc(Vec_IO_DP &zahyo,DP Len,DP R,DP rt,DP alp,int Prt,int num,DP phi,int D,bool &minus, bool &invalidflag,bool &IPRerror);
    void ModeCalc(Vec_IO_DP &zahyo,DP Len,DP R,DP rt,DP alp,int Prt1,int Prt2,int num,DP phi,int D,bool &minus, bool &invalidflag,bool &IPRerror);
    void deriveEigvec2D(Vec_I_DP &zahyo,DP Lx,DP Ly,DP R,DP rt,DP alp,int Prt,Vec_O_DP &out,DP &eigval,int D,int vecnum);
    ...
    void outputPEL2D(Vec_I_DP &zahyo,DP Lx,DP Ly,DP R,DP rt,DP alp,int Prt,int num,DP phi,Vec_I_DP &eigvec,int D,int vecnum,DP func(Vec_IO_DP,DP,DP,DP,DP,int,int,DP));
  ...
  namespace dim3
  {
    void ModeCalc3D(Vec_IO_DP &zahyo,DP Len,DP R,DP rt,DP alp,int Prt,int num,DP phi,int D,bool &minus, bool &invalidflag,bool &IPRerror);
    void ModeCalc3D(Vec_IO_DP &zahyo,DP Len,DP R,DP rt,DP alp,int Prt1,int Prt2,int num,DP phi,int D,bool &minus, bool &invalidflag,bool &IPRerror);
    void deriveEigvec3D(Vec_I_DP &zahyo,DP Lx,DP Ly,DP Lz,DP R,DP rt,DP alp,int Prt,Vec_O_DP &out,DP &eigval,int D,int vecnum);
    ...
    void outputPEL3D(Vec_I_DP &zahyo,DP Lx,DP Ly,DP Lz,DP R,DP rt,DP alp,int Prt,int num,DP phi,Vec_I_DP &eigvec,int D,int vecnum,DP func(Vec_IO_DP,DP,DP,DP,DP,DP,int,int,DP));
...
void JT::dim2::ModeCalc(Vec_IO_DP &zahyo,DP Len,DP R,DP rt,DP alp,int Prt,int num,DP phi,int D,bool &minus, bool &invalidflag,bool &IPRerror)
{
  if(checkdim2(D)) {
    minus=true;
    int N=zahyo.size();
    ...
    invalidflag=false;
    JT::dim2::Hesspot(zahyo,Hes,Len,R,rt,alp,Prt,D,eps);
    ...
    for(int j=0;j<N;j++)//固有値がすげえ負ならばそのときのPELを描いてもらう。
      if(Eigval[j]<-EPS)
	minus=false;
    JT::WriteEigInfo(EigMat,Eigval,num,phi,D,IPRerror);
  }
}
void JT::dim2::ModeCalc(Vec_IO_DP &zahyo,DP Len,DP R,DP rt,DP alp,int Prt1,int Prt2,int num,DP phi,int D,bool &minus, bool &invalidflag,bool &IPRerror)
{
  if(checkdim2(D)) {
    minus=true;
    int N=zahyo.size();
    ...
    DP EPS=numeric_limits<double>::epsilon(),halfm,eps=1.0;
    invalidflag=false;
    JT::dim2::Hesspot(zahyo,Hes,Len,R,rt,alp,Prt1,Prt2,D,eps);
    NR::jacobi(Hes,Eigval,EigMat,cnt,invalidflag);
    if(invalidflag==true)
      return;
    if(rt!=1.0) {
    ...
    for(int j=0;j<N;j++)//固有値がすげえ負ならばそのときのPELを描いてもらう。
      if(Eigval[j]<-EPS)
	minus=false;
    JT::WriteEigInfo(EigMat,Eigval,num,phi,D,Prt1,Prt2,IPRerror);
  }
}
...
void JT::dim3::ModeCalc3D(Vec_IO_DP &zahyo,DP Len,DP R,DP rt,DP alp,int Prt,int num,DP phi,int D,bool &minus,bool &invalidflag,bool &IPRerror)
{
  if(checkdim3(D)) {
    minus=true;
    int N=zahyo.size();
    ...
    DP EPS=numeric_limits<double>::epsilon(),halfm,eps=1.0;
    invalidflag=false;
    JT::dim3::Hesspot3D(zahyo,Hes,Len,R,rt,alp,Prt,D,eps);
    NR::jacobi(Hes,Eigval,EigMat,cnt,invalidflag);
    if(invalidflag==true)
      return;
...
void JT::dim3::ModeCalc3D(Vec_IO_DP &zahyo,DP Len,DP R,DP rt,DP alp,int Prt1,int Prt2,int num,DP phi,int D,bool &minus,bool &invalidflag,bool &IPRerror)
{
  if(checkdim3(D)) {
    minus=true;
    int N=zahyo.size();
    ...
    DP EPS=numeric_limits<double>::epsilon(),halfm,eps=1.0;
    invalidflag=false;
    JT::dim3::Hesspot3D(zahyo,Hes,Len,R,rt,alp,Prt1,Prt2,D,eps);
    NR::jacobi(Hes,Eigval,EigMat,cnt,invalidflag);
    if(invalidflag==true)
      return;
...
void JT::WriteEigInfo(Mat_I_DP &EigMat,Vec_I_DP &Eigval,int num,DP phi,int D,bool &IPRerror)
  //----------------------------------------------------------
  ...
  //----------------------------------------------------------
{
  ...
  JT::CollectiveMotionCheck(EigMat,CMnumber,D);// IPR をCMnumberに格納
  JT::ssInput(ss,num);
  JT::ssInput(ss2,phi);
  if(num<10)
  ...
  for(int i=0;i<N;i++) {
	...
	if(CMnumber[i]>0.7) {
	  ...
	  cout<<setprecision(10)<<Eigval[i]<<' '<<sqrt(Eigval[i])<<' '<<CMnumber[i]<<' '<<cm<<endl;
	  cout<<"=========================="<<endl;
	}
	cm-=cmdist;
      }
	...
	eg=fabs(Eigval[i]);
	wrmode<<setprecision(10)<<eg<<' '<<sqrt(eg)<<' '<<CMnumber[i]<<' '<<'0'<<endl;
      }
    }
}
...
void JT::WriteEigInfo_test(Mat_I_DP &EigMat,Vec_I_DP &Eigval,int num,DP phi,int D,bool &IPRerror)
...
//同一固有値ならば"*"を先頭に打たない.
{
  ...
  string wrname;
  ofstream wrmode;
  DP cmdist=1.0/N,cm=1.0,eigtmp=Eigval[0],EPS=numeric_limits<double>::epsilon();
  ...
  if(num<10)
    wrname ="EigCumtest0000"+ss.str()+'_'+ss2.str()+".txt";
  else if(num>=10 && num<100)
    wrname ="EigCumtest000"+ss.str()+'_'+ss2.str()+".txt";
  else if(num>=100 && num<1000)
    wrname ="EigCumtest00"+ss.str()+'_'+ss2.str()+".txt";
  else if(num>=1000 && num<10000)
    wrname ="EigCumtest0"+ss.str()+'_'+ss2.str()+".txt";
  else
    wrname ="EigCumtest"+ss.str()+'_'+ss2.str()+".txt";
  wrmode.open(wrname.data());
...
//vecnumは0~N-1まで!!
void JT::deriveEigvec(Vec_I_DP &zahyo,DP Len,DP R,DP rt,DP alp,int Prt,Vec_O_DP &out,DP &eigval,int D,int vecnum)
{
  int N=zahyo.size();
  ...
  if(D==2) {
    JT::dim2::Hesspot(zahyo,Hes,Len,R,rt,alp,Prt,D,1.0);
  }
  else if(D==3) {
    JT::dim3::Hesspot3D(zahyo,Hes,Len,R,rt,alp,Prt,D,1.0);
  }
  NR::jacobi(Hes,Eigval,EigMat,cnt,invalidflag);
  if(invalidflag) {
    return;
...
void JT::deriveEigvec(Vec_I_DP &zahyo,DP Len,DP R,DP rt,DP alp,int Prt1,int Prt2,Vec_O_DP &out,DP &eigval,int D,int vecnum)
{
  int N=zahyo.size();
  if(vecnum>N-1 || vecnum<0) {
    cerr<<"cannot derive Eigenvector..."<<endl;
  ...
  if(D==2) {
    JT::dim2::Hesspot(zahyo,Hes,Len,R,rt,alp,Prt1,Prt2,D,1.0);
  }
  else if(D==3) {
    JT::dim3::Hesspot3D(zahyo,Hes,Len,R,rt,alp,Prt1,Prt2,D,1.0);
  }
  NR::jacobi(Hes,Eigval,EigMat,cnt,invalidflag);
  if(invalidflag) {
    return;
...
void JT::dim2::deriveEigvec2D(Vec_I_DP &zahyo,DP Lx,DP Ly,DP R,DP rt,DP alp,int Prt,Vec_O_DP &out,DP &eigval,int D,int vecnum)
{
  int N=zahyo.size();
  if(vecnum>N-1 || vecnum<0) {
    cerr<<"cannot derive Eigenvector..."<<endl;
  ...
  if(D==2) {
    JT::dim2::Hesspot(zahyo,Hes,Lx,Ly,R,rt,alp,Prt,D,1.0);
  }
  NR::jacobi(Hes,Eigval,EigMat,cnt,invalidflag);
  if(invalidflag) {
    return;
...
void JT::dim3::deriveEigvec3D(Vec_I_DP &zahyo,DP Lx,DP Ly,DP Lz,DP R,DP rt,DP alp,int Prt,Vec_O_DP &out,DP &eigval,int D,int vecnum)
{
  int N=zahyo.size();
  if(vecnum>N-1 || vecnum<0) {
    cerr<<"cannot derive Eigenvector..."<<endl;
  ...
  if(D==3) {
    JT::dim3::Hesspot3D(zahyo,Hes,Lx,Ly,Lz,R,rt,alp,Prt,D,1.0);
  }
  NR::jacobi(Hes,Eigval,EigMat,cnt,invalidflag);
  if(invalidflag) {
    return;
...
void JT::outputPEL(Vec_I_DP &zahyo,DP Len,DP R,DP rt,DP alp,int Prt,int num,DP phi,Vec_I_DP &eigvec,int D,int vecnum,DP func(Vec_IO_DP,DP,DP,DP,int,int,DP))
{
  int N=zahyo.size();
  Vec_DP Relrzahyo(N);
  ...
  for(int j=0;j<2*jmax;j++)//50000はテキトーな区間幅
    {
      tmpfactor=(j-jmax)*0.00000001*Len*sqrt(D*N);
      //0.00000001は適当に設定.
      for(int i=0;i<N;i++)
	Relrzahyo[i]=zahyo[i]+tmpfactor*eigvec[i];
      wrPEL<<0+tmpfactor<<' '<<setprecision(15)<<func(Relrzahyo,Len,R,rt,alp,Prt,D)<<endl;
    }
}
void JT::outputPEL(Vec_I_DP &zahyo,DP Len,DP R,DP rt,DP alp,int Prt1,int Prt2,int num,DP phi,Vec_I_DP &eigvec,int D,int vecnum,DP func(Vec_IO_DP,DP,DP,DP,int,int,int,DP))
{
  int N=zahyo.size();
  Vec_DP Relrzahyo(N);
  stringstream ss,ss2,ss3;
  ...
  for(int j=0;j<2*jmax;j++)//50000はテキトーな区間幅
    {
      ...
      for(int i=0;i<N;i++)
	Relrzahyo[i]=zahyo[i]+tmpfactor*eigvec[i];
      wrPEL<<0+tmpfactor<<' '<<setprecision(15)<<func(Relrzahyo,Len,R,rt,alp,Prt1,Prt2,D)<<endl;
    }
}
void JT::dim2::outputPEL2D(Vec_I_DP &zahyo,DP Lx,DP Ly,DP R,DP rt,DP alp,int Prt,int num,DP phi,Vec_I_DP &eigvec,int D,int vecnum,DP func(Vec_IO_DP,DP,DP,DP,DP,int,int,DP))
{
  int N=zahyo.size();
  Vec_DP Relrzahyo(N);
  stringstream ss,ss2,ss3;
  ...
  for(int j=0;j<2*jmax;j++)//50000はテキトーな区間幅
    {
      ...
      for(int i=0;i<N;i++)
	Relrzahyo[i]=zahyo[i]+tmpfactor*eigvec[i];
      wrPEL<<0+tmpfactor<<' '<<setprecision(15)<<func(Relrzahyo,Lx,Ly,R,rt,alp,Prt,D)<<endl;
    }
}
...
void JT::dim3::outputPEL3D(Vec_I_DP &zahyo,DP Lx,DP Ly,DP Lz,DP R,DP rt,DP alp,int Prt,int num,DP phi,Vec_I_DP &eigvec,int D,int vecnum,DP func(Vec_IO_DP,DP,DP,DP,DP,DP,int,int,DP))
{
  int N=zahyo.size();
  Vec_DP Relrzahyo(N);
  stringstream ss,ss2,ss3;
  ...
  for(int j=0;j<2*jmax;j++)//50000はテキトーな区間幅
    {
      ...
      for(int i=0;i<N;i++)
	Relrzahyo[i]=zahyo[i]+tmpfactor*eigvec[i];
      wrPEL<<0+tmpfactor<<' '<<setprecision(15)<<func(Relrzahyo,Lx,Ly,Lz,R,rt,alp,Prt,D)<<endl;
    }
}
