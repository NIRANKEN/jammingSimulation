...
int main(int argc,char* argv[])
{
  ...
  else if(PNum>128 && PNum<=pow(2.0,19)) {
  ...
  if(dim==2)//2D method
      ...
      for(int l=sd_s;l<sd_e;l++)//sample loop
	{
	  ...
	  JT::InitializeSimulationparameter(PHI,PHIstart,pluseig,notrelaxeig,loopcnt,CRadBI,Leng,ratio,PNum,Pratio1,Pratio2,dim,Rad,relaxation_times);
	  ...
	  for(int i=0;i<99999;i++)//PHI loop
	    {
	      ...
	      cout<<"PN:"<<PNum<<", sample:"<<l<<", PHI:"<<PHI<<", 緩和にかかった時間:"<<relaxation_times<<endl;
	      if(loopcnt>0)
	  ...
	  for(int i=0;i<99999;i++)//99999はテキトー.
	    {
	      ...
	      JT::noise(zahyo,Rad,Leng,n_strength);
	      // cout<<"===TEST==="<<endl;
	      // cout<<DMPt<<", "<<MASS<<", "<<Tstep<<", "<<ftol<<", "<<Leng<<", "<<Leng<<", "<<Leng<<", "<<Rad<<", "<<ratio<<", "<<alpha<<", "<<eps<<", "<<Pratio1<<", "<<Pratio2<<", "<<l<<", "<<PHI<<", "<<fra_inc<<", "<<fra_dec<<", "<<fra_alp<<", "<<dmpt_init<<", "<<tstep_max<<", "<<ITFIRE<<", "<<dim<<", "<<VViter<<endl;
	      // cout<<"===TEST==="<<endl;
	      ...
	      relaxation_times+=VViter;
	      cout<<"PN:"<<PNum<<", sample:"<<l<<", PHI:"<<PHI<<", 緩和にかかった時間:"<<relaxation_times<<", ENG:"<<pot<<endl;
	      if(loopcnt>0)
		JT::loopErrWrite(sserr,l,i,PHI,pot,ftol,relaxation_times,n_strength);
	      if(pot<fftol && fabs(DeltaPHI)>1.0e-6) {
		//一個前の座標とPHIに戻す
	      ...
	      else
		{
		  ...
		  cout<<"pluseig:"<<pluseig<<", iprerror:"<<iprerror<<", jacobi_invalid:"<<invalidflag<<endl;
