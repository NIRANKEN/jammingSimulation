...
//-------------------------
	...
	void jacobi(Mat_IO_DP &a, Vec_O_DP &d, Mat_O_DP &v, int &nrot,bool &invalidflag);
	...
	void ModeCalc(Vec_IO_DP &zahyo,DP Len,DP R,DP rt,DP alp,int Prt,int num,DP phi,int D,bool &minus,bool &invalidflag);
      	void ModeCalc3D(Vec_IO_DP &zahyo,DP Len,DP R,DP rt,DP alp,int Prt,int num,DP phi,int D,bool &minus,bool &invalidflag);
	...
	void deriveEigvec3D(Vec_I_DP &zahyo,DP Len,DP R,DP rt,DP alp,int Prt,Vec_O_DP &out,DP &eigval,int D,int vecnum);
	...
	void tapping(Vec_IO_DP& EigDispvec,DP eigval,DP eng,DP EPS,DP R,DP disp_ratio);

	void WriteEigInfo(Mat_I_DP &Eigmat,Vec_I_DP &Eigval,int num,DP phi);
	void WriteEigInfo_test(Mat_I_DP &Eigmat,Vec_I_DP &Eigval,int num,DP phi);
	...
	void noise2(Vec_IO_DP& p,DP R,DP n_disp_ratio);
	...
	DP CtoDerivePHI2D(DP L,DP rt,DP PN,int Prt,int D);
	DP CtoDerivePHI3D(DP L,DP rt,DP PN,int Prt,int D);
	void ssInput(stringstream &ss,int a);
	void ssInput(stringstream &ss,DP a);
	void loopErrWrite(stringstream &ss,int snum,int iter,DP PHI,DP pot,DP ftol,int VViter,DP noise_strength);
	void jacobiErrWrite(stringstream &ss,int snum,int iter,DP PHI,DP pot,DP ftol,int VViter);
	void backtoPreviousPHI2D(Vec_DP &z,Vec_I_DP &ztmp,DP &PHI,DP &DPHI,DP &R,DP C,DP &Tmax,int & loop, int D);
	void backtoPreviousPHI3D(Vec_DP &z,Vec_I_DP &ztmp,DP &PHI,DP &DPHI,DP &R,DP C,DP &Tmax,int &loop,int D);
	void forwardtoPHI2D(Vec_I_DP &z,Vec_DP &ztmp,DP &PHI,DP DPHI,DP &R,DP C,int &loop,int D);
	void forwardtoPHI3D(Vec_I_DP &z,Vec_DP &ztmp,DP &PHI,DP DPHI,DP &R,DP C,int &loop,int D);
	void PeriodicMove(Vec_DP &zahyo,DP L);
